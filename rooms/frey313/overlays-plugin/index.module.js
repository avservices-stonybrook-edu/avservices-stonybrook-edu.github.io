/*!
 * PhotoSphereViewer.OverlaysPlugin 5.7.4
...
import { BoxGeometry, Mesh, MeshBasicMaterial, SphereGeometry, Vector2 as Vector22, VideoTexture } from "three";
...
var OverlaysPlugin = class extends AbstractConfigurablePlugin {
  ...
  __createCubeMesh(config, { texture, panoData }) {
    ...
    for (let i = 0; i < 6; i++) {
      if (panoData.flipTopBottom && (i === 2 || i === 3)) {
        texture[i].center = new Vector22(0.5, 0.5);
  ...
  async __addSphereImageOverlay(config) {
    ...
    const applyPanoData = panoData?.isEquirectangular && utils.isNil(config.yaw) && utils.isNil(config.pitch) && utils.isNil(config.width) && utils.isNil(config.height);
    let texture;
    if (applyPanoData) {
      const adapter = this.__getEquirectangularAdapter();
