/*!
 * PhotoSphereViewer.PlanPlugin 5.7.4
...
// src/PlanPlugin.ts
import { AbstractConfigurablePlugin, events, utils as utils2 } from "@photo-sphere-viewer/core";

// src/components/PlanComponent.ts
import { AbstractComponent as AbstractComponent2, CONSTANTS as CONSTANTS2, utils } from "@photo-sphere-viewer/core";
...
var AbstractPlanButton = class extends AbstractComponent {
...
var PlanCloseButton = class extends AbstractPlanButton {
  ...
  update() {
    this.container.innerHTML = this.plan.collapsed ? map_default : CONSTANTS.ICONS.close;
...
// src/components/PlanComponent.ts
var PlanComponent = class extends AbstractComponent2 {
  constructor(viewer, plugin) {
    super(viewer, {
      className: `psv-plan ${CONSTANTS2.CAPTURE_EVENTS_CLASS}`
    });
    this.plugin = plugin;
    this.state = {
      visible: false,
  ...
  handleEvent(e) {
    if (utils.getClosest(e.target, `.${CONSTANTS2.CAPTURE_EVENTS_CLASS}:not(.psv-plan)`)) {
      return;
    }
    switch (e.type) {
...
var getConfig = utils2.getConfigParser(
  ...
  },
  {
    ...
    position: (position, { defValue }) => {
      return utils2.cleanCssPosition(position, { allowCenter: false, cssOrder: true }) || defValue;
    },
    bearing: (bearing) => utils2.parseAngle(bearing),
    buttons: (buttons, { defValue }) => ({ ...defValue, ...buttons })
  }
);
var PlanPlugin = class extends AbstractConfigurablePlugin {
  ...
  init() {
    super.init();
    utils2.checkStylesheet(this.viewer.container, "plan-plugin");
  ...
  setOptions(options) {
    ...
    if (!utils2.isNil(options.bearing)) {
  ...
  setHotspots(hotspots) {
    ...
    hotspots?.forEach((hotspot) => {
      if (!hotspot.id) {
        ...
        utils2.logWarn(`Duplicated hotspot id "${hotspot.id}`);
      } else {
        ids.push(hotspot.id);
      }
    });
  ...
  __markersToHotspots(markers) {
    return markers.filter((marker) => marker.data?.[MARKER_DATA_KEY]).map((marker) => {
      ...
      if (!hotspot.coordinates) {
        utils2.logWarn(`Marker #${marker.id} "plan" data is missing GPS coordinates`);
        return null;
      }
      return hotspot;
    }).filter((h) => h);
